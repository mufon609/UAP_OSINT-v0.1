...
| table_cell_word_budget | ✅ | Foundational from `af5f789` — budget started at 50; bumped to 55 at `44272af` (warning-cleanup pass cleared 2 of 5 then-current warnings firing on legitimate-sized cells). Hardcoded fallback was stale (still 50) until C17 audit synced it to schema's current 55. Operational behavior was correct via ctx.schema lookup; only the fallback path lagged. Markdown link wraps stripped before counting so cross-reference-heavy cells don't false-positive. |
