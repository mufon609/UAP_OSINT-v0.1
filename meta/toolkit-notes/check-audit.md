...
| required_sections | ✅ | Foundational from `af5f789` — schema-driven dispatcher (archetype/kind routing + type-default fallback) + corpus-addendum extension mechanism both shipped in initial commit. Stable check code; schema's per-type required_sections lists evolved via F.1a–F.6 renderer landings. Earlier instance of the schema-as-single-source-of-truth pattern that iff_section later formalized for conditional_keys. Corpus addenda dormant in this instance (aawsap-dird template exists; no node uses `corpus:` fm); forward-defensive for fork targets. Match semantics permissive (exact OR prefix-with-qualifier); boundary check catches structural divergence so no real loophole. |
| schema_version_compat | ✅ | Foundational from `af5f789` — schema_version compat was inline in validate.py from day one. Migration: `60bb88d` (C11 lift); `2f3effb` (BACKLOG #8 — consolidated `schema_version_compat_messages` into lib._common). Stable foundational thin-wrapper on shared helper. Four call sites now share one source of truth: this check + artifact_top_level + governance_files (template + governance-doc routes). |
...
| status_archetype_kind | ✅ | Foundational from `af5f789`. Presence-guarded enum validation on status / archetype / kind. C17 audit caught a layering bug: the ORIGINAL implementation used truthy guards (`if fm.get("archetype"):`) which skipped null values, contradicting the layering claim made in frontmatter_required + id_path_match docstrings. C17 fixed the guards to presence-based and corrected the cross-referenced docstrings. Zero corpus impact (no null fields existed); future protection improved. |
| table_cell_word_budget | ✅ | Foundational from `af5f789` — budget started at 50; bumped to 55 at `44272af` (warning-cleanup pass cleared 2 of 5 then-current warnings firing on legitimate-sized cells). C17 audit removed the silent fallback (was masking schema drift — the af5f789 hardcoded 50 stayed lagged for ~3 weeks after schema bumped to 55) per BACKLOG C21. Now uses direct dict access; KeyError surfaces if schema is missing the value. Markdown link wraps stripped before counting. |
...
| contracts | ✅ | F.5a (`8d377c0`) — organization renderer schema. Renderer-coupled-defensive (Primary Contracts table on gov-contractor node bodies needs structured data). Heaviest entry-list shape investigated: 4 required + 6 optional fields. Custom logic: deliverables list-of-paths check + primary_counterparty_path leading-slash. Currently exercises 2 corpus artifacts (arlo-solutions + sancorp-consulting). BACKLOG C6 tracks the c10/c11 value→note migration deferred until B1 ships. |
...
| key_personnel | ✅ | F.5a (`8d377c0`). CLOSED leadership_class enum {director, deputy, staff, advisor, other} drives renderer sub-grouping (Directors / Deputy Leadership / Other). OPTIONAL with graceful fallback (absent → Other) — softer than participants.capacity (required); design accommodates org sources that don't specify hierarchical role. role is free-text (same pattern as affiliations.role). |
| location_relationships | ✅ | F.6a (`5a67ec1`). UNIQUE: only relationship_entry shape allowing heterogeneous target types (location → anything). Other relationship-style checks constrain target type (person↔person, org↔org, person↔org). Free-text `relationship` field (not enum) because heterogeneous targets create combinatorial complexity closed enum can't capture cleanly. Check validates entity_path shape only; type validity via link_resolution's broken-link registry. |
...
| org_relationships | ✅ | F.5a (`8d377c0`). CLOSED relationship_type enum {parent, subsidiary, predecessor, successor, contractor, contracting-agency, partner, other} — same pattern as participants.capacity. BACKLOG B1 primary site: corpus has multiple AARO + UAPTF cases where evidentiary nuance lives in `.note` (DoD-vs-OUSD parent disambiguation, AOIMSG-vs-UAPTF predecessor, IPMO co-location-vs-partner, EXCOM-as-oversight) but the renderer drops the .note field. Check itself is correct; gap is in downstream table renderer. |
| ownership_timeline | ✅ | F.6a (`5a67ec1`). Shape-only validation (period_start + owner + use_status required); date format deferred to render-time `parse_date_tuple` 9999 sentinel; chronological ordering enforced downstream by chronological_tables. Shares period_start/period_end pattern with 3 sibling checks (key_personnel, contracts, affiliations) — research-queue tracks pending "known start, unknown end (but not ongoing)" convention surfaced by Phillips Deputy Director kp2 case. |
| participants | ✅ | F.2a (`13a2859`) — co-introduced with witnesses_testimony but broader scope (both event kinds, not just hearing). CLOSED capacity enum {witness-eyewitness, witness-whistleblower, witness-institutional, committee-member, observer, official, other}; capacity drives mechanical sub-grouping in the F.2 hearing renderer. Distinct from media_versioning's extensible-with-warn `aspect` — capacity's `other` is a fixed miscellaneous bucket, not an extensibility escape. |
...
| program_involvement | ✅ | F.1a (`8007ef1`) — institutional-actor archetype. Closed enums shared with vouching_chain: evidentiary_basis {primary-source, sworn-testimony, on-record, self-attested, secondary} + confidence {high, medium, low}. Common evidentiary-quality vocabulary across credibility-attestation cross-references. |
...
| publication_record | ✅ | F.1a (`8007ef1`) — reporter archetype. NO enums (distinct from the three sibling F.1a archetype-conditional checks). Shape-only: publication / outlet / date are source-attested free-text fields. Reporter archetype defers vocabulary to the publishing record itself. |
...
| relationships | ✅ | F.1b (`491e6f3`) — sibling of affiliations, both required for the F.1b person renderer's Confirmed/Flagged-split tables. person→person constrained target type; relationship is free-text (same rationale as location_relationships.relationship — diversity of human relationship vocabulary). Completes the relationship-shape taxonomy: person↔person (relationships), person→org (affiliations), org↔org (org_relationships), location→anything (location_relationships). |
...
| speakers | ✅ | F.3b (`6cb131a`). Universal-on-transcript (both kinds; same kind-agnostic scope as participants is to events). Cross-reference surface; statements live in quotes per statements-only discipline. role is free-text (same pattern as affiliations.role + key_personnel.role) and explicitly OUT of prose-drift scope per the 2026-04-21 second scope cut. |
| timeline | ✅ | Anchored to F.1a (`8007ef1`) — one of six entry-list checks landed together. Foundational shape; stable since. The category field is free-text by design — the original `timeline_category_values` schema list + claimed-but-unimplemented warn was removed in C17 cleanup as cargo-cult schema decoration (no consumer; corpus organically extended past the suggested nine; the field always operated as free-text). Schema is now honest about the field's role. |
| top_scope_activity | ✅ | F.6a (`5a67ec1`) — sibling of ownership_timeline + location_relationships. Topic-specific scope filter (institutional topic-scope only; popular paranormal lore goes in rumors). Period-bearing pattern shared with 4 sibling checks. Surfaced BACKLOG C22 (topic-neutrality drift): the field name was originally `uap_scope_activity` and the section header was hardcoded `## UAP-Scope Activity`, contradicting the toolkit's "schema is topic-neutral" scope claim. C22 closeout (commit `ffad3ed`) renamed the field to `top_scope_activity` (and `uap_relevance` → `top_relevance` on person artifacts); the renderer composes the section header from `meta/topic/overview.md::display_name` so this fork still renders `## UAP-Scope Activity` while a different fork renders e.g. `## Cold-War-Document-Scope Activity`. |
