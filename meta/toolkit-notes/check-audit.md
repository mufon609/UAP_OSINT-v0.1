...
| uap_scope_activity | ✅ | F.6a (`5a67ec1`) — sibling of ownership_timeline + location_relationships. Topic-specific scope filter (institutional UAP-scope only; popular paranormal lore goes in rumors). Period-bearing pattern shared with 4 sibling checks. Surfaced BACKLOG C22 (topic-neutrality drift): the field name + section header `## UAP-Scope Activity` are explicitly UAP-named, contradicting the toolkit's "schema is topic-neutral" scope claim. Same applies to `uap_relevance` on person artifacts. |
