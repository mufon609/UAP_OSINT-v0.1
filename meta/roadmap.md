...
| Step E.1 — Pre-commit / CI hook | ✅ | `scripts/tests/pre-commit.sh` chains the gate sequence (originally 5 at E.1 close: help-check / smoke / validate.py / validate-research.py / build-state.py --check; subsequently extended to 7 with test_stopwords + review-coverage.py) |
