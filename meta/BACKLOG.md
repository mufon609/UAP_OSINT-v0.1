...
### C4 — Investigate the orchestrator-authored handoff overriding role contracts

In the `/build` pipeline, role subagents cannot hand work to each other (a
subagent can't spawn a subagent), so every handoff routes through the main-thread
orchestrator: each role returns a stub and the orchestrator **authors the next
role's prompt** from it. Failure mode: the orchestrator re-derives and injects
evidentiary / linking / relevance policy into those prompts, and because a
subagent weights the just-issued task prompt above its standing system-prompt
contract (`worker.md` / `builder.md` / `build-protocol`), the improvised
instruction silently overrides the contract. Concrete instance: a document build's
authored Description dropped every source-attested entity link (historical
scientists + institutions named in the prose) after the orchestrator told the
worker to "flag … for the builder to judge" and the builder to "do not over-link;
judge load-bearing-ness" — importing the quote-selection relevance judgment
(discretionary) into entity linking (absolute: stub-never-null). It recurred
across sessions (a prior session over-explained the wrong thing the same way), so
it is a structural property of how the orchestrator delegates, not a one-off.

Investigate before changing anything:
(a) Was the handoff thinner in the past? Check git history / `prompts/topology.md`
    lineage — did an earlier design pass each role's stub through more directly (a
    fixed minimal handoff) vs. today's free-prose orchestrator-authored prompt? If
    a leaner handoff existed and worked, why did it grow?
(b) What is the minimal, closed input set each role actually needs (internal /
    external investigator, archive, worker, builder, auditor) so the orchestrator
    can relay inputs + prior stubs and never author policy? The contracts already
    own policy (stub-never-null: `build-protocol` ~96–106, `worker.md` ~67–77,
    `builder.md` ~72–76) and are correct — do not change them.
(c) Where to encode the fix so a future orchestrator does the right thing by
    default: clear, positive, closed per-step input specs in
    `.claude/skills/build/SKILL.md` (state exactly what to pass each role) — not a
    defensive guardrail, not a new validator/gate.
(d) Whether anything beyond linking is overridden by orchestrator prose (quote
    scope, relevance, prose-drift framing).

Produce a recommended SKILL.md revision (and any topology.md note), then edit and
record the rationale. Do NOT add/remove a validator/gate. The node instance that
exposed this is already corrected — this item is the process fix only.
