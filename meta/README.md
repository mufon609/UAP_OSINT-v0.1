...
| `build-state.md` | Auto-generated corpus snapshot (every node + status + archetype/kind) — regenerated by `scripts/build/build-state.py`; never hand-edited |
...
- **Root** — stable governance specs + forward-looking work registers
  ...
  `BACKLOG.md`, `roadmap.md`. Topic-neutral; survives a fork. Plus
  `build-state.md`, the one generated file at this tier — a corpus
  snapshot whose content is topic-specific but which regenerates from
  node frontmatter (a fork rebuilds it empty via
