...
streamed interviews — three classes of entity must be registered
as `entities_referenced[]` entries with `wrap_path` AND must appear
as `[`/path`]` body wraps somewhere in the node (typically inside
the corresponding `timeline[].event` text):

- **Venue** — the organization that hosts or distributes the
  appearance. WEAPONIZED registers as `/organizations/weaponized`;
  Joe Rogan Experience as `/organizations/joe-rogan-experience`;
  the Sol Foundation Symposium as `/organizations/sol-foundation`.
- **Host / interviewer / moderator** — the person conducting the
  appearance. George Knapp on WEAPONIZED; Joe Rogan on JRE; the
  moderator of a Sol Foundation panel. The host is structurally
  distinct from the subject of the appearance.
- **Transcript-to-be** — the transcript node where the verbatim
  evidence will live, registered with its forward-looking
  `wrap_path` even before the transcript node is built. The
  broken-link registry surfaces the unbuilt transcript as a Phase 2
  build candidate.
