...
1. **`Agent(internal-investigator)`** with the target. Read its stub:
   ...
   - **all-internal branch:** if `all_internal: true` / `gaps: []`, skip
     steps 2–3; scaffold from the reused sources and go to step 4.
...
8. **Done** when the auditor reports `health: pass` and
   `adjacent_needs_update: []`. Report the built node and a short summary of
   each role's returned stub. (Stubs are return values — the orchestrator reads
   each as it goes; nothing is written to disk for the handoff.)
