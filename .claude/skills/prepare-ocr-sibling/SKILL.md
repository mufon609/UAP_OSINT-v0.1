...
2. **Produce the VLM page-image read.** Dispatch **`Agent(ocr-page-producer)`**,
   ...
   **output directory** (`.scratch/drafts/ocr-{stem}/`). The producer contract owns the rest —
...
3. **Write the sibling + confirm it against PaddleOCR.** Pass the **per-page
   ...
   python3 scripts/tools/ocr-consensus.py run sources/{category}/{stem}.pdf \
       ...
       > .scratch/drafts/ocr-{stem}/confirm.txt 2>&1
   ```
   ...
   regenerates an existing sibling (backfill). `Read` `.scratch/drafts/ocr-{stem}/confirm.txt`. A
   ...
   **`Agent(ocr-page-verifier)`**, one per disjoint page range that carries flagged
   ...
   path** (`.scratch/drafts/ocr-{stem}/confirm.txt`), and the **source PDF path**. The verifier
