...
# The worker WRITES its fragment to /tmp/fragment-{slug}-{source stem}.yaml
...
fragment_path: /tmp/fragment-{slug}-{stem}.yaml
...
```yaml
# worker — fragment FILE (/tmp/fragment-{slug}-{source stem}.yaml, one per source)
