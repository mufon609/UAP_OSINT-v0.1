...
| `diarize-audio.py` | Speaker diarization via pyannote.audio's `speaker-diarization-3.1`. Output is identity-blind (`SPEAKER_00` / `SPEAKER_01` / …); pair with `detect-faces.py` baselines + `stitch-transcript.py` to map labels to identities. Slow on CPU (real-time); use `--start` / `--end` to slice to the contested passage. Project-local venv at `.venv-diarize/` populated by `setup-diarize-audio.sh`. |
| `stitch-transcript.py` | Bridges three independent pipeline artifacts (downloaded auto-caption transcript + diarize `segments.csv` + identity baselines) into a single speaker-labeled transcript at `/tmp/stitch-{slug}/stitched.md`. Per-segment frame extraction + face detection + face-embedding match → aggregate per `SPEAKER_NN` → emit speaker label at turn boundaries. Confidence ratings (`high` / `medium` / `low` / `none`) per resolved speaker; explicit "Manual review required" banner when any speaker resolves below `high`. Contributor uses the output to populate `speaker_id` on transcript-artifact quotes. (Diarize+stitch is the pre-agent-pipeline path; the agent-based `/prepare-transcript-sibling` is now the default.) |
...
| `setup-diarize-audio.sh` | One-time idempotent installer for the audio-side diarize step: creates `.venv-diarize/`, installs `pyannote.audio` + `torch` + `torchaudio`, walks contributor through the Hugging Face user-conditions acceptance + `HF_TOKEN` setup. Separate from `setup-photo-identity.sh` because the audio path is optional and carries different dependencies. |
