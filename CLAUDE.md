...
| `stitch-transcript.py` | Bridges three independent pipeline artifacts (downloaded auto-caption transcript + diarize `segments.csv` + identity baselines) into a single speaker-labeled transcript at `/tmp/stitch-{slug}/stitched.md`. Per-segment frame extraction + face detection + pHash match → aggregate per `SPEAKER_NN` → emit speaker label at turn boundaries. Confidence ratings (`high` / `medium` / `low` / `none`) per resolved speaker; explicit "Manual review required" banner when any speaker resolves below `high`. Contributor uses the output to populate `speaker_id` on transcript-artifact quotes. |
| `setup-photo-identity.sh` | One-time idempotent installer for the visual-side video pipeline: `python3-opencv`, `opencv-data`, `yt-dlp`, `ffmpeg`/`ffprobe`, a JS runtime. Reports any missing pieces; re-runnable. |
