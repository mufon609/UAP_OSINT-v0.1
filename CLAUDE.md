...
| `check-vocab.py` | Pre-flight vocabulary check for prose-drift discipline — pools an artifact's primary-source significant tokens (mirroring `validate-research.py`'s tokenizer) and reports per-input-token presence. Contributor convenience for drafting `description` / `background` / `uap_relevance` / `credibility_notes` / synthesis-content `.note` fields against source vocabulary. |
