...
| `spot-check-attribution.py` | Mechanical turn-by-turn cross-check of a finished speaker-attribution sibling against the source video. Samples beg/mid/end frames per turn, runs the dlib HOG + embedding engine (via `detect-faces.py`) against the photo-identity-log baselines, and flags each turn `confirmed` / `contested-fold` (another in-transcript speaker seen — the fold-up signal) / `contested-other` / `inconclusive`. CSV + stdout summary; `--embed-threshold` (default 0.50). Auto-relaunches under `.venv-face/`. |
