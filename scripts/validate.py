...
_extraction_type_cache = None  # path str (relative to sources/) -> extraction_type
...
def _load_extraction_types():
    ...
    Lazy + cached for the duration of one validator run. Returns an empty
    dict on parse failure or missing manifest (extract_source_text falls
...
def extract_source_text(source_path):
    ...
    For PDFs flagged `extraction_type: ocr-scan` in sources/manifest.yaml,
    prefer a same-stem `.txt` sibling (contributor-produced clean
    transcription) over pdftotext output. The PDF's text layer was
    generated by upstream OCR and carries character-level corruptions
    that pdftotext faithfully reproduces; the .txt sibling restores the
    document's actual content as visually verified against the PDF.
    Falls back to pdftotext if no sibling exists or the read fails.
    """
