...
| `stub-reconcile.py` | Duplicate-stub reconcile aid — computes the complete coined-stub set (built nodes ∪ every artifact's `associated_entities` / path fields / inline `[`/…`]` wraps) and surfaces clusters of distinct slugs that may name ONE entity, for contributor judgment. Closes the build's stub-reuse blind spot: the reuse survey sees only *built* nodes, so two artifacts can coin divergent slugs for the same unbuilt entity (`/people/v-teofilo` vs `/people/vincent-teofilo`). Two NER-free rules within a type — *initials* (same surname + initials-compatible leading tokens, the people case) and *subset* (one token set ⊂ the other, the org abbreviation case, generic-word-guarded). Sweep mode (corpus-wide clusters) or `--name "Vincent Teofilo"` coinage query (does a stub already exist? — consulted at build time). Read-only, never a gate (same-surname-different-person is legitimate); covers `people` + `organizations` by default (locations excluded — part-whole place pairs are not duplicates). |
