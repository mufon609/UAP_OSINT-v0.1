...
exceeds the schema's ``limits.table_cell_words_soft`` budget (default
50). The budget is a soft guideline pointing at cells that should
promote to a ``###`` subsection or a finding node — not a hard error;
warnings only.
...
``_table_cell_overages`` (only consumer was this check).
"""
...
def check(ctx):
    budget = ctx.schema.get("limits", {}).get("table_cell_words_soft", 50)
