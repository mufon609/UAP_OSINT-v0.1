...
the ``_research_utils`` helpers shared with the other entry-list
checks (check_unique_ids / check_lifecycle_fields /
require_source_dict).

Note: ``period_start`` / ``period_end`` format isn't validated here.
Date parsing happens at render time
