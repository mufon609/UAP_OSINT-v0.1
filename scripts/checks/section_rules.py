...
section:
  ...
  - ``split: [...]``: section must carry the listed H3 subsections.
    ...
    omitted when empty by convention (INVESTIGATOR.md,
    "Reading a node's evidentiary state"), so the walker skips
    Flagged even when the schema's split rule names it.
