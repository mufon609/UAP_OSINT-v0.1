...
def main():
    ...
    print()
    print("Next — this is /build step 4 (scaffold), run by the orchestrator.")
    print("Scaffold the research artifact, then let the role pipeline take over:")
    print(f"  python3 scripts/build/research-scaffold.py --target {TYPE_DIRS[args.type]}/{args.slug} \\")
    print(f"      --sources <ALL reuse + archived paths, relative to sources/>")
    print("The orchestrator then continues to the sibling gate -> worker -> builder")
    print("(see .claude/skills/build/SKILL.md \"The shape\"). Do NOT hand-run")
    print("build-from-research.py — the builder role renders the node body.")
