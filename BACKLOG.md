...
### 30. IPMO Sancorp contracts — description summarization decision deferred

`/organizations/ipmo` was built 2026-05-02 with `gov` kind. Per schema,
the `Primary Contracts` section is required only on `gov-contractor`
artifacts; gov-kind orgs reference contracts via `org_relationships`
(IPMO uses one `contractor` entry pointing to Sancorp, with full
contract detail living on `/organizations/sancorp-consulting`).

The IPMO Sancorp contract chain (HQ003422C0064 / HQ003423C0061 /
HQ003424C0046) has structural relevance to IPMO that is currently
under-surfaced on the IPMO node:

- **HQ003422C0064** (period of performance June 10, 2022 – June 9,
  2023; $3,026,577.70) — first IPMO Support Services contract; PWS
  released via FOIA 23-F-0906; carries §1.2 chain-of-command and §1.3
  Objectives attestations that already render as IPMO Key Passages.
- **HQ003423C0061** (June 10, 2023 – June 9, 2024; $3,021,082) —
  follow-on for "Specialized and Sensitive Administrative, Policy,
  Operations, and Analytic Support Services."
- **HQ003424C0046** (June 10, 2024 – May 27, 2026; $5,751,921) —
  current contract; doubled in value over predecessors.

**Decision deferred this session.** Description prose enumeration
would duplicate Sancorp Primary Contracts table content. Timeline
already carries all three contract awards. The cross-reference via
`org_relationships.contractor → /organizations/sancorp-consulting`
points readers to the canonical contract detail.

**Question for a future session:** does any IPMO-specific framing
warrant description-prose summarization beyond what timeline +
Sancorp cross-reference already deliver? Candidate angles:
- Contract value escalation pattern (HQ003424C0046 = ~1.9× the
  preceding contract value)
- IPMO PWS scope evolution across the three contracts (what changed
  vs. what stayed the same — the FOIA 23-F-0906 release covers
  HQ003422C0064 only; later PWSs not retrieved)
- Parallel-construction observation per the structural-coupling
  handoff: same Sancorp prime contract chain at IPMO (Jun 2022 / May
  2023 / May 2024) and AARO (Sep 2022 / Aug 2024) — already documented
  on AARO and Sancorp; whether the IPMO description should mirror

**Scope.** ~30 minutes if the answer is "add one paragraph"; longer
if the answer involves retrieving HQ003423C0061 and HQ003424C0046
PWSs (likely FOIA-bound — both are 8(A) Sole Source via WHS).

**Priority.** Low. Not a correctness gap; the contract chain is
documented on Sancorp + Timeline. Description-level redundancy is
the open question.

Surfaced: IPMO node build, 2026-05-02 — user explicitly directed
"only in relationships(unless you want to summarize some or need to
to build the node)" with subsequent-session review.
