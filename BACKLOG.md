...
passages"; Step 5 "fill the node body only from the scratchpad";
Step 6 "cross-reference with `[/path]`"). This is the pre-Phase-II
workflow. For the 6 renderer-supported types (document / person /
event / transcript / media / organization / location) node bodies are
regenerated by `scripts/build-from-research.py` from the research
artifact, not hand-authored. Only finding nodes still follow the
direct-authoring path, pending F.7.

The document self-frames as "summary for reference; the authoritative
procedure lives in `prompts/build.md`" (line 85), so it's not
technically wrong — but the summary itself is out of sync with current
reality, and new contributors reading it top-to-bottom may follow the
stale walkthrough.

**Fix:** restructure Step 1–10 around the artifact-first workflow
(scaffold artifact → populate per Phase I → regenerate node →
coverage review). Relegate the hand-authored path to a single
"Finding nodes (hand-authored until F.7)" subsection. Keep the
Hard rules + source-read-first framing at the top — those are
still current.

Surfaced: 2026-04-21 staleness audit verification pass.
